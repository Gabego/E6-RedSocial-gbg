const postControllers = require('./posts.controlles')
...
const getPostById = (req, res) => {
    ...
    .then(data => {
       if(data){
        ...
        res.status(404).json({message: 'Inavlid ID: '+id})
       }
...
const postNewPost = (req, res) => {
    ...
    postControllers.createPosts({userId, content})
    .then(data => {
         res.status(200).json(data)
     })
     .catch(err => {
         res.satus(400).json({message: err.message, fields: {
            content:'text'
         }})
     })
}
...
const patchPost = (req, res) => {
    const {content} = req.body
    const id = req.params.id
    postControllers.updatePosts(id, userId, {content})
